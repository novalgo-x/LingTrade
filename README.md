...
> 各接口的积分要求与调用频次限制见 [Tushare 积分说明](https://tushare.pro/document/1?doc_id=290)。**推荐 5000 积分**：可覆盖基础数据、日线行情、财务数据，以及增减持、龙虎榜、融资融券等参考数据，满足绝大部分分析场景；120 积分（免费）也能完成基于日线行情的基础技术面分析，积分越高可用数据越全。
